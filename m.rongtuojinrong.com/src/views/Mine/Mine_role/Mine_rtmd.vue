<template>
  <div class="rtjrBox">
    ...
    <div class="rtjrBox-1">
      ...
      <div v-if="isShow" class="rtjrBox-1c">{{unreceived_principal}}</div>
      ...
      <div class="rtjrBox-1d">
          <div class="rtjrBox-1d1">
              ...
              <span v-if="isShow">{{unreceived_interest}}</span>
              <span v-else>****</span>
          </div>

          <div class="rtjrBox-1d1">
              <span>已赚收益（元）</span>
              <span v-if="isShow">{{received_profit}}</span>
              <span v-else>****</span>
          </div>
    ...
    <div class="rtjrBox-2">
        <router-link class="rtjrBox-2a" :to="{name: 'mdcreditor'}" tag="div">
        ...
        <router-link :to="{name:'mdhk'}" class="rtjrBox-2a" tag="div">
            <div class="rtjrBox-2a1">
                <div class="rtjrBox-2a1a">
...
export default {
    data(){
        return{
            ...
            popupVisible: false,
            popupVisible1: false,
            unreceived_principal: 0,
            unreceived_interest: 0,
            received_profit: 0
        }
    },
    components:{
    ...
    methods:{
        ztShow(){
            ...
            if(this.isShow){
                localStorage.setItem("mymdstatus",1);
            }
            if(!this.isShow){
                localStorage.setItem("mymdstatus",0);
            } 
        },
        popupShow() {
    ...
    created(){
        ...
        this.$post('/rongtuojinrong/userCenter/mindeng/getIncome.json')
        .then(res=>{
            this.$toast.clear();//关闭toast
            this.unreceived_principal = res.data.data.unreceived_principal;
            this.unreceived_interest = res.data.data.unreceived_interest;
            this.received_profit = res.data.data.received_profit;
        })
        .catch(err=>{
        ...
        //从本地缓存中取状态
        if(localStorage.getItem("mymdstatus")==1){
            this.isShow = true;
        }
        if(localStorage.getItem("mymdstatus")==0){
            this.isShow = false;
        }         
    }
}
